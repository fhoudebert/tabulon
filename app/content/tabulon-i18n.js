...
const DICT = {
    en: {
        ...
        'prefs.chatHow': 'Generate one below, then send it ONCE to the people you play with, by any channel except the relay itself — a message, an e-mail. They paste it into the same field. From then on your games with them are protected without any further step.',
        'prefs.chatWarning': 'One key for everyone you play with means everyone who has it could read your other conversations, if they knew which game to look at. A game can be given a key of its own from its chat window. Replacing the key below makes every earlier conversation unreadable — including for you.',
    ...
    fr: {
        ...
        'prefs.chatHow': 'Générez-en une ci-dessous, puis envoyez-la UNE FOIS aux personnes avec qui vous jouez, par n’importe quel canal sauf le relai lui-même — un message, un courriel. Elles la collent dans le même champ. Vos parties avec elles sont ensuite protégées sans autre geste.',
        'prefs.chatWarning': 'Une seule clé pour tout le monde signifie que chacun de ceux qui l’ont pourrait lire vos autres conversations, s’il savait quelle partie regarder. Une partie peut recevoir une clé qui lui est propre depuis sa fenêtre de discussion. Remplacer la clé ci-dessous rend illisibles toutes les conversations précédentes — y compris pour vous.',
