...
(function() {
		...
		function loadResources(type,who,callback){
			...
			function checkLoaded(){
				...
				if (nbRes==0){
					...
					// Order matches the converted .gltf's material order
					// (piecetop, pieceborders) -- not the legacy JSON
					// source's order (pieceborders, piecetop), which got
					// reversed during the glTF conversion. AdaptGltfToGeoMat
					// assigns group.materialIndex by first-appearance order
					// in the .gltf, so this array must follow that order
					// too, or the wrong texture ends up on the wrong part
					// of the piece.
					var pieceMat=[mattop,matborder];
	...
	View.Game.xdInit = function(xdv) {
		...
		xdv.createGadget("lightside", {
			...
			"turtles3d": {
				create: function() {
					var backlight = new THREE.PointLight( 0xaaaaff, 3 * (window.JOCLY_LIGHT_FACTOR || Math.PI), 30 , 0);
					return backlight;
				},
			},
		});
		xdv.createGadget("lightback", {
			"3d": {
				type: "custom3d",
				create: function() {
					var backlight = new THREE.PointLight( 0xaaccff, 0.7 * (window.JOCLY_LIGHT_FACTOR || Math.PI), 30, 0 );
					return backlight;
				},
				z: 10000,
				y: -10000,
			...
			"turtles3d" : {
				create: function() {
					var backlight = new THREE.PointLight( 0xaaccff, 3 * (window.JOCLY_LIGHT_FACTOR || Math.PI), 30 , 0);
					return backlight;
				},
			},
		});
		...
		/*function createSkyball(style){
			...
			for ( var i = 0; i < posAttr.count; i++ ) 
			{
				...
				if(style=="turtles3d"){
					...
					colors[i*3] = colors[i*3+1] = colors[i*3+2] = delta/6;
				}
			}
			...
			xdv.createGadget("piece#"+index, {
				...
				"alquerque3d":{
					...
					create: function() {
						...
						// No longer needed on BufferGeometry: with no
						// .groups defined, every triangle already renders
						// with material index 0 by default (the single
						// material in the array below). The old per-face
						// `.materialIndex = 0` loop only made sense on the
						// removed THREE.Geometry, which stored faces with
						// their own materialIndex property.
				...
				(function(p){
					avatar.getResource(url,function(geometry , materials){
						...
						if(p==0){
							...
							materials0[0].specular = new THREE.Color(0x020202);
						materials0[0].specular.convertSRGBToLinear();
			...
			"turtles3d":{
				...
				create: function() {
					...
					this.getResource("smoothedfilegeo|"+smooth+"|"+fullPath+"/res/xd-view/meshes/rainbowflat.js",function(geometry , materials) {
						...
						materials[0].specular = new THREE.Color(0x222222);
						materials[0].specular.convertSRGBToLinear();
			...
			"turtles3d" : {
				...
				create: function() {					
					...
					this.getResource("smoothedfilegeo|"+0+"|"+fullPath+"/res/xd-view/meshes/turtle-fences.js",function(geometry , materials) {
						...
						var fenceMat = new THREE.MeshPhongMaterial({color : 0xffffff , specular:0x222222 , shininess:100, flatShading:true});
...
//# sourceMappingURL=brazilian-draughts-view.js.map
